# We have observed some issues that programs just `end` after an install command, not printing any errors. 
# This might be related to docker issues, i.E. I don't see any exceptions from Useagent
# One project where this was observed was Guava, which tried to install the jdk first. 
# The behavior to be observed is that it should try to install java first, which might take a while. 
